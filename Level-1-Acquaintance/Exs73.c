...
static_assert('z'-'a' == 25, "alphabetic characters are not contiguous");
...
int main(int argc, char* argv[argc+1]) {
  static unsigned const base[] = { 0, 8, 10, 16, 36, 37, };
  for (unsigned j = 0; j < sizeof base/ sizeof base[0]; ++j) {
    printf("%u:", base[j]);
    for (int i = 1; i < argc; ++i) {
      unsigned long res = Strtoul(argv[i], base[j]);
